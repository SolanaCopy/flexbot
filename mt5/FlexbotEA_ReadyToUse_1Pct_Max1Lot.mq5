//+------------------------------------------------------------------+
//| FlexbotEA_ReadyToUse_1Pct_Max1Lot.mq5                            |
...
void OnTradeTransaction(const MqlTradeTransaction& trans, const MqlTradeRequest& request, const MqlTradeResult& result)
{
  ...
  // If EA restarted, try recover last signal id from persisted file
  if(g_lastSignalId=="") {
    PersistLoad();
    if(Trim(g_persistedSignalId)!="") g_lastSignalId = g_persistedSignalId;
  }
  if(g_lastSignalId=="") { if(InpDebugTrade) Print("ClosePost skip: signal_id empty (no persisted state)"); return; }
  ...
  if((ulong)magic != InpMagic) { if(InpDebugTrade) Print("ClosePost skip: magic mismatch dealMagic=", magic); return; }
  ...
  if(g_lastClosedSignalId == g_lastSignalId) { if(InpDebugTrade) Print("ClosePost skip: already posted for signal_id=", g_lastSignalId); return; }
  ...
  if(InpDebugHttp) Print("POST /signal/closed code=", st, " outcome=", outcome, " result=", resultStr, " signal_id=", g_lastSignalId);
  if(st >= 200 && st < 300) {
    g_lastClosedSignalId = g_lastSignalId;
